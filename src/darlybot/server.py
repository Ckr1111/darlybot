...
<html lang=\"ko\">
  ...
  <body>
    <h1>Darlybot Helper</h1>
    <p>로컬 서버가 실행 중입니다. 아래 엔드포인트를 통해 상태를 확인하거나 곡 이동을 요청할 수 있습니다.</p>
    <ul>
      <li><a href=\"/ping\"><code>GET /ping</code></a> – 서버 상태 확인</li>
      <li><a href=\"/songs\"><code>GET /songs</code></a> – 로드된 곡 목록(JSON)</li>
      <li><code>POST /navigate</code> – 곡 번호 또는 제목을 전달하여 이동</li>
    </ul>
    <p>현재 곡 데이터: <code>{song_count}</code>곡 · <code>{csv_path}</code></p>
    <footer>
      <p>연결 대상: <code>http://{host}:{port}</code></p>
    </footer>
  </body>
</html>
"""
...
class SongServer:
    ...
    def _render_homepage(self) -> str:
        ...
        return _HOME_TEMPLATE.format(
            ...
            csv_path=html.escape(str(self.index.csv_path)),
            song_count=len(self.index),
        )
    ...
    def _build_handler(self):
        ...
        class Handler(BaseHTTPRequestHandler):
            ...
            def _write_html(self, markup: str, status: HTTPStatus = HTTPStatus.OK) -> None:
                payload = markup.encode("utf-8")
                self._set_headers(status, content_type="text/html; charset=utf-8")
                self.wfile.write(payload)
            ...
            def do_GET(self) -> None:
                ...
                if path in {"", "/"}:
                    self._write_html(server._render_homepage())
                elif path == "/ping":
